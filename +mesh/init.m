...
%   precision is lost and the number is rounded.
    ...
    if nargin == 1
        ...
        elseif endsWith(varargin{1}, '.mat')
            M = mesh.io.load_mat(varargin{1});
        elseif endsWith(varargin{1}, '.ascii')
            M = mesh.io.load_ascii(varargin{1});
        else
        ...
        if args.laplace
            ...
            % the desired order
            [M.S, M.A, M.Al] = mesh.proc.FEM_higher(M, args.laplaceorder);
        end
